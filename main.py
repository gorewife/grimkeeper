...
from botc.discord_utils import safe_defer, safe_send_interaction, safe_send_message
...
async def get_active_players(
    ...
    try:
        ...
        if channel and bot.session_manager and channel.category:
            session = await bot.session_manager.get_session_from_channel(
...
async def send_temporary(
    ...
    delay: float = DELETE_DELAY_NORMAL,
) -> discord.Message:
...
async def toggle_prefix(
    member: discord.Member, channel: discord.TextChannel, prefix_key: str
):
    prefixes = {
        "brb": PREFIX_BRB,
        "st": PREFIX_ST,
    ...
    if not bot_member:
        await send_temporary(
            channel,
            "⚠️ Bot configuration error. Please contact an admin.",
            delay=DELETE_DELAY_ERROR,
        )
        return
    ...
    if prefix_key == "st":
        ...
        if active["st"]:
            if session and bot.session_manager:
                ...
                logger.info(
                    f"User {member.display_name} used *st but no session exists in category {botc_category.name}. Suggest using /setbotc."
                )
            ...
            if botc_category:
                ...
                for other_member in guild.members:
                    ...
                    if (
                        other_member.nick
                        and other_member.nick.startswith(PREFIX_ST)
                        and other_member.id in category_members
                    ):
                        try:
                            new_other_nick = other_member.nick[len(PREFIX_ST) :]
                            bot_initiated_nick_changes.add(
                                (other_member.id, new_other_nick)
                            )
                            await other_member.edit(nick=new_other_nick)
                        except Exception as e:
                            logger.warning(
                                f"Could not remove ST prefix from {other_member.display_name}: {e}"
                            )
    ...
    confirmation_messages = {
        "brb": f"{member.display_name} will be right back"
        if active.get("brb")
        else f"{member.display_name} is back",
        "cost": "You are now Co-Storyteller!"
        if active.get("cost")
        else "No longer Co-Storyteller.",
        "spe": "You're now spectating"
        if active.get("spe")
        else "You're no longer spectating",
    }
...
def check_rate_limit(
    user_id: int, command: str, cooldown_seconds: int = COMMAND_COOLDOWN_SECONDS
) -> bool:
    now = time.time()
...
async def call_townspeople(
    guild: discord.Guild, category_id: Optional[int] = None
) -> tuple[int, discord.VoiceChannel]:
    guild_id = guild.id
    ...
    if not session:
        raise ValueError(
            "❌ No session found for this category. An admin should run `/setbotc` to create a session first."
        )
    ...
    if not dest_channel:
        raise ValueError(
            "❌ Town Square not configured for this session. An admin should run `/settown #channel` from within this category."
        )

    if not botc_category:
        raise ValueError(
            "❌ Could not find BOTC category. Please check the category configuration."
        )

    can_move, _ = check_bot_permissions(guild)
    if not can_move:
        raise ValueError(
            "Bot lacks 'Move Members' permission. Please grant this permission in Server Settings."
        )
        ...
        for member in channel.members:
            ...
            try:
                if (
                    ...
                    and dest_channel
                    and member.voice.channel.id == dest_channel.id
                ):
                    continue
            ...
            try:
                if (
                    member.voice
                    and member.voice.channel
                    and member.voice.channel.id in exception_ids
                ):
                    continue
            except AttributeError as e:
                logger.debug(
                    f"Exception channel check failed for {member.display_name}: {e}"
                )
...
async def get_session_from_channel_wrapper(channel, session_manager):
    ...
    if (
        not session_manager
        or not channel
        or not hasattr(channel, "category")
...
async def start_game_handler(
    interaction: discord.Interaction, script: object, custom_name: str = ""
):
...
async def stats_handler(interaction: discord.Interaction) -> None:
    """Display game stats for the server
    ...
    if not check_rate_limit(interaction.user.id, "stats", COMMAND_COOLDOWN_LONG):
        await interaction.response.send_message(
            "⏳ Please wait before using /stats again.", ephemeral=True
        )
        return
    ...
    try:
        ...
        if not history:
            await interaction.response.send_message(
                "No game history recorded for this server yet.", ephemeral=True
            )
            return
        ...
        if not history:
            await interaction.response.send_message(
                "No valid game history found. Invalid games have been cleaned up.",
                ephemeral=True,
            )
            return
        ...
        balance = (
            f"{EMOJI_BALANCE} Balanced"
            if abs(good_rate - evil_rate) < 10
            else (
                f"{EMOJI_GOOD} Good-favored"
                if good_rate > evil_rate
                else f"{EMOJI_EVIL} Evil-favored"
            )
        )
        ...
        if len(history) >= 10:
            ...
            trend = (
                "📈 Good trending up"
                if recent_good_pct > good_rate + 10
                else (
                    "📉 Evil trending up"
                    if recent_good_pct < good_rate - 10
                    else "📊 Stable"
        ...
        await safe_send_interaction(
            interaction, "❌ Failed to load stats. Database error.", ephemeral=True
        )
    except Exception as e:
        logger.exception(f"Unexpected error in stats_handler: {e}")
        await safe_send_interaction(
            interaction, "❌ Failed to load stats. See logs.", ephemeral=True
        )


async def gamehistory_handler(
    interaction: discord.Interaction, limit: int = 50
) -> None:
    """Display recent game history for the server with pagination.
    ...
    if not check_rate_limit(interaction.user.id, "gamehistory", COMMAND_COOLDOWN_LONG):
        await interaction.response.send_message(
            "⏳ Please wait before using /gamehistory again.", ephemeral=True
        )
        return
    ...
    try:
        ...
        history = await db.get_game_history(
            guild_id, limit=limit, category_id=category_id
        )

        if not history:
            scope_msg = "this session" if category_id else "this server"
            await interaction.response.send_message(
                f"No game history recorded for {scope_msg} yet.", ephemeral=True
            )
            return
        ...
        await safe_send_interaction(
            interaction,
            "❌ Failed to load game history. Database error.",
            ephemeral=True,
        )
    except Exception as e:
        logger.exception(f"Unexpected error in gamehistory_handler: {e}")
        await safe_send_interaction(
            interaction, "❌ Failed to load game history. See logs.", ephemeral=True
        )
...
class GameHistoryView(discord.ui.View):
    ...
    def create_embed(self) -> discord.Embed:
        ...
        for game in page_games:
            ...
            embed.add_field(name=script_display, value=field_value, inline=False)

        embed.set_footer(
            text=f"Page {self.current_page + 1}/{self.total_pages} • v{VERSION}"
        )
...
async def storytellerstats_handler(
    ...
    if not check_rate_limit(
        interaction.user.id, "storytellerstats", COMMAND_COOLDOWN_LONG
    ):
        await interaction.response.send_message(
            "⏳ Please wait before using this command again.", ephemeral=True
        )
        return
    ...
    try:
        ...
        if not stats:
            if user:
                await interaction.response.send_message(
                    "No storyteller statistics recorded yet.", ephemeral=True
                )
            else:
                await interaction.response.send_message(
                    "No storyteller statistics recorded for this server yet.",
                    ephemeral=True,
                )
            return
        ...
        if user:
            ...
            if not stats:
                await interaction.response.send_message(
                    f"{user.display_name} has no storytelling history.", ephemeral=True
                )
                return
        ...
        if user and len(stats) == 1:
            ...
            if total == 0:
                await interaction.response.send_message(
                    f"{user.display_name} has no completed games yet.", ephemeral=True
                )
                return
        ...
        await safe_send_interaction(
            interaction,
            "❌ Failed to load storyteller stats. Database error.",
            ephemeral=True,
        )
    except Exception as e:
        ...
        await safe_send_interaction(
            interaction,
            "❌ Failed to load storyteller stats. See logs.",
            ephemeral=True,
        )
...
async def deletegame_handler(interaction: discord.Interaction, game_id: int) -> None:
    try:
        ...
        if not is_admin(member):
            try:
                await interaction.response.send_message(
                    "Only administrators can delete game history.", ephemeral=True
                )
        ...
        if not deleted:
            try:
                await interaction.response.send_message(
                    f"❌ Game ID `{game_id}` not found. Use `/gamehistory` to see valid game IDs.",
                    ephemeral=True,
                )
            except discord.HTTPException:
                try:
        ...
        try:
            await interaction.response.send_message(
                f"✅ Deleted game ID `{game_id}`", ephemeral=True
            )
        except discord.HTTPException:
            try:
                await interaction.followup.send(
        ...
        await safe_send_interaction(
            interaction, "❌ Failed to delete game. Database error.", ephemeral=True
        )
    except Exception as e:
        logger.exception(f"Unexpected error in deletegame_handler: {e}")
        await safe_send_interaction(
            interaction, "❌ Failed to delete game. See logs.", ephemeral=True
        )
...
async def clearhistory_handler(interaction: discord.Interaction) -> None:
    try:
        ...
        if not is_admin(member):
            await safe_send_interaction(
                interaction,
                "Only administrators can clear game history.",
                ephemeral=True,
            )
            return
        ...
        if not history:
            await safe_send_interaction(
                interaction, "No game history to clear.", ephemeral=True
            )
            return
        ...
        try:
            ...
            await safe_send_interaction(
                interaction,
                "❌ Failed to clear history. Database error.",
                ephemeral=True,
            )
            return

        await safe_send_interaction(
            interaction, "✅ Cleared game history for this server.", ephemeral=True
        )
    except Exception as e:
        logger.exception(f"Unexpected error in clearhistory_handler: {e}")
        await safe_send_interaction(
            interaction, "❌ Failed to clear history. See logs.", ephemeral=True
        )
...
async def deleteshortgames_handler(
    ...
    try:
        ...
        if not is_admin(member):
            await safe_send_interaction(
                interaction, "Only administrators can delete games.", ephemeral=True
            )
            return

        if minutes < 1:
            await safe_send_interaction(
                interaction, "Duration must be at least 1 minute.", ephemeral=True
            )
            return
        ...
        try:
            ...
            await safe_send_interaction(
                interaction,
                "❌ Failed to delete games. Database error.",
                ephemeral=True,
            )
            return
        ...
        if count == 0:
            await safe_send_interaction(
                interaction,
                f"No games found shorter than {minutes} minute(s).",
                ephemeral=True,
            )
        else:
            await safe_send_interaction(
                interaction,
                f"✅ Deleted {count} game(s) shorter than {minutes} minute(s).",
                ephemeral=True,
            )
    except Exception as e:
        logger.exception(f"Unexpected error in deleteshortgames_handler: {e}")
        await safe_send_interaction(
            interaction, "❌ Failed to delete games. See logs.", ephemeral=True
        )
...
async def autosetup_handler(interaction: discord.Interaction) -> None:
    ...
    try:
        ...
        status_msg = await interaction.followup.send(
            "🕯️ Setting up your Blood on the Clocktower server..."
        )
        ...
        existing_botc_categories = [
            ...
            if cat.name.lower().startswith("🩸• blood on the clocktower")
            or cat.name.lower() in ["botc", "bot c", "blood on the clocktower"]
        ]
        ...
        botc_category = await guild.create_category(category_name, position=0)
        ...
        consultation = await guild.create_voice_channel(
            "🕯️┃Consultation", category=botc_category, user_limit=0
        )

        town_square = await guild.create_voice_channel(
            "🏛️┃Town Square", category=botc_category, user_limit=0
        )

        private_2 = await guild.create_voice_channel(
            "🌙┃Private Chamber (2)", category=botc_category, user_limit=2
        )

        private_3 = await guild.create_voice_channel(
            "⚰️┃Private Chamber (3)", category=botc_category, user_limit=3
        )

        commons = await guild.create_voice_channel(
            "🗡️┃Commons", category=botc_category, user_limit=0
        )
        ...
        if session_manager:
            try:
                ...
                logger.info(
                    f"Autosetup: Created session for guild {guild_id}, category {botc_category.id}"
                )
            except Exception as e:
        ...
        if session_number == 1:
            ...
            description = (
                f"Your Blood on the Clocktower session is ready in **{category_name}**!"
            )

        embed = discord.Embed(title=title, description=description, color=gothic_red)
        ...
        await status_msg.edit(
            content=f"✅ Setup complete! Check {announce_channel.mention} for details."
        )
        ...
        try:
            await interaction.followup.send(
                "❌ Setup failed due to database error. Please try again.",
                ephemeral=True,
            )
        except discord.HTTPException:
        ...
        try:
            await interaction.followup.send(
                "❌ Setup failed. Check bot permissions and try again.", ephemeral=True
            )
        except discord.HTTPException:
...
async def load_cogs():
    ...
    try:
        ...
        logger.info(
            "Loaded all cogs: events, slash, polls, timers, voice_commands, commands"
        )
    except Exception as e:
